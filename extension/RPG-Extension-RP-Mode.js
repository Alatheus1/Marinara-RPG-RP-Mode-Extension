  ...
  if (syncRow) {
    ...
    if (syncBtn) marinara.on(syncBtn, "click", function () {
      ...
      Object.keys(abilitiesByCat).forEach(function (catId) {
        ...
        arr.forEach(function (ab) {
          if (!ab || !ab.id) return;
          jobs.push(upsertAbilityLorebookEntry(ab, charId, catId).then(function (entryId) {
            if (entryId && entryId !== ab.lorebookEntryId) {
              ab.lorebookEntryId = entryId;
              return true;
            }
            return false;
          }).catch(function (e) {
            log("re-sync: upsert failed for " + (ab.name || ab.id), e && e.message);
            return null;
          }));
        });
      });
      if (!jobs.length) {
        setSyncMsg("No charms on the sheet to sync.", "warn");
        return;
      }
