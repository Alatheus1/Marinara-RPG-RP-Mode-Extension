...
function addBackground() {
  ...
  /* Phase 7 — open the trait dialog for the new background. The Add
     path no longer pushes a blank row inline; the user fills out name
     + dots + description + mechanical in the dialog and the entry only
     materialises on Save (mirrors how charms and items work). */
  openBackgroundDialog(null, def);
}
...
function removeBackground(idx) {
  ...
  arr.splice(idx, 1);
  saveSheet(state.chatId, state.sheet);
  /* Phase 7 — clean up the associated chat-lorebook entry so the
     description doesn't keep injecting after the trait is gone. */
  if (removed && removed.lorebookEntryId) {
    deleteSheetEntryLorebook(removed).catch(function (e) {
      log("background remove: lorebook delete failed", e && e.message);
    });
  }
  renderSheet();
}

/* Phase 7 — Background edit dialog. New (no idx) → fresh draft; edit
   (idx) → clones the entry into a draft. On Save, writes back to
   state.sheet.backgrounds[idx] and upserts the Character Reference
   lorebook entry. */
function openBackgroundDialog(idx, defaultValue) {
  var cfg = state.ruleset.backgrounds || {};
  var lo = (typeof cfg.min === "number") ? cfg.min : 0;
  var hi = (typeof cfg.max === "number") ? cfg.max : 5;
  var existing = (typeof idx === "number" && Array.isArray(state.sheet.backgrounds))
    ? state.sheet.backgrounds[idx] : null;
  var draft = existing
    ? JSON.parse(JSON.stringify(existing))
    : {
        name: "",
        value: (typeof defaultValue === "number") ? defaultValue : (typeof cfg["default"] === "number" ? cfg["default"] : 0),
        description: "",
        mechanical: "",
        lorebookKeyword: "",
        lorebookEntryId: ""
  ...
  openTraitDialog({
    title: existing ? "Edit Background" : "New Background",
    kind: "background",
    draft: draft,
    fields: [
      { key: "name",        label: "Name",  type: "text",     placeholder: "Resources, Allies, Mentor, …" },
      { key: "value",       label: "Dots",  type: "number",   min: lo, max: hi, "default": (typeof cfg["default"] === "number" ? cfg["default"] : 0) },
      { key: "description", label: "Description",         type: "textarea", placeholder: "What this background represents in the fiction (the wealthy ally, the inherited estate, the mentor's reach)." },
      { key: "mechanical",  label: "Mechanical",          type: "textarea", placeholder: "Mechanical effect — Resources 3 = comfortable middle class with $40k disposable, Allies 3 = three trusted contacts who can be called once per story…" },
      { key: "lorebookKeyword", label: "Keyword", type: "text",   placeholder: "(auto-fills from name on save)" }
    ],
    onSave: function (saved) {
      if (!Array.isArray(state.sheet.backgrounds)) state.sheet.backgrounds = [];
      if (existing) {
        state.sheet.backgrounds[idx] = saved;
      } else {
        state.sheet.backgrounds.push(saved);
      }
      saveSheet(state.chatId, state.sheet);
      renderSheet();
      var charId = state.activeCharacterId;
      upsertSheetEntryLorebook("background", saved, charId).then(function (entryId) {
        if (entryId && entryId !== saved.lorebookEntryId) {
          saved.lorebookEntryId = entryId;
          /* Find the entry in the array (push may have moved indices) and
             persist the new lorebookEntryId. */
          var arr = state.sheet.backgrounds;
          for (var i = 0; i < arr.length; i++) {
            if (arr[i] === saved || (arr[i] && arr[i].name === saved.name && arr[i].value === saved.value && !arr[i].lorebookEntryId)) {
              arr[i].lorebookEntryId = entryId;
              break;
            }
          }
          saveSheet(state.chatId, state.sheet);
        }
      }).catch(function (e) { log("background upsert failed", e && e.message); });
    }
  });
}
...
/* Phase 7 — generic edit dialog for sheet "trait" entries (backgrounds,
   merits, flaws). Fields are declared per-call so the same dialog
   services every trait section. On save, the entry is updated in place
   on state.sheet and upserted to the chat Character Reference lorebook
   via upsertSheetEntryLorebook so the AI sees the description +
   mechanical text the next time the keyword fires in narration.

   opts = {
     title:    "Edit Background" | "New Merit" | ...,
     kind:     "background" | "merit" | "flaw",
     draft:    cloned entry object (caller serialises any state copy),
     fields:   ordered array of { key, label, type, placeholder?, min?,
               max?, options? }  — type ∈ "text" | "number" | "select" |
               "textarea",
     onSave:   function(saved) — called after state.sheet write + lorebook
               upsert; receives the persisted entry,
     onCancel: optional close handler
   } */
function openTraitDialog(opts) {
  if (!opts || !opts.draft || !Array.isArray(opts.fields)) return;
  if (state.traitDialogEl && state.traitDialogEl.parentNode) {
    state.traitDialogEl.parentNode.removeChild(state.traitDialogEl);
    state.traitDialogEl = null;
  }
  var backdrop = marinara.addElement(document.body, "div", { "class": "mrrp-dialog-backdrop mrrp-dialog-backdrop--open" });
  if (!backdrop) return;
  state.traitDialogEl = backdrop;
  var dialog = marinara.addElement(backdrop, "div", { "class": "mrrp-dialog" });
  if (!dialog) { document.body.removeChild(backdrop); state.traitDialogEl = null; return; }

  marinara.addElement(dialog, "h3", { textContent: String(opts.title || "Edit") });

  var draft = opts.draft;
  var fieldInputs = {};

  opts.fields.forEach(function (f) {
    var row = marinara.addElement(dialog, "div", { "class": "mrrp-item-form__row" });
    if (!row) return;
    marinara.addElement(row, "label", { textContent: f.label || f.key });
    var input;
    var curVal = draft[f.key];
    if (f.type === "select") {
      input = marinara.addElement(row, "select", { "class": "mrrp-item-form__select" });
      if (input && Array.isArray(f.options)) {
      ...
      input = marinara.addElement(row, "input", {
        "class": "mrrp-item-form__input",
        type: "number",
        value: String(typeof curVal === "number" ? curVal : (typeof f["default"] === "number" ? f["default"] : 0))
      });
      if (input) {
      ...
      input = marinara.addElement(row, "textarea", {
        "class": "mrrp-item-form__textarea",
        placeholder: f.placeholder || ""
      });
      if (input) input.value = (typeof curVal === "string") ? curVal : "";
    } else {
      input = marinara.addElement(row, "input", {
        "class": "mrrp-item-form__input",
        type: "text",
        value: (typeof curVal === "string") ? curVal : "",
        placeholder: f.placeholder || ""
      });
    }
    fieldInputs[f.key] = { el: input, def: f };
  });
  ...
  function close() {
    if (backdrop && backdrop.parentNode) backdrop.parentNode.removeChild(backdrop);
    if (state.traitDialogEl === backdrop) state.traitDialogEl = null;
  ...
  if (saveBtn) marinara.on(saveBtn, "click", function () {
    /* Validate name first — every trait needs one. The Save button
       refuses an empty name and surfaces the reason inline. */
    var nameEntry = fieldInputs.name;
    var nameVal = nameEntry && nameEntry.el ? (nameEntry.el.value || "").trim() : "";
    if (!nameVal) {
      if (msg) {
    ...
    /* Pull each field value back into the draft, coercing types. */
    Object.keys(fieldInputs).forEach(function (k) {
      var entry = fieldInputs[k];
      if (!entry || !entry.el) return;
      var def = entry.def;
      if (def.type === "number") {
        var n = parseInt(entry.el.value, 10);
        if (isNaN(n)) n = (typeof def["default"] === "number") ? def["default"] : 0;
        if (typeof def.min === "number" && n < def.min) n = def.min;
        if (typeof def.max === "number" && n > def.max) n = def.max;
        draft[k] = n;
      } else if (def.type === "select") {
        draft[k] = entry.el.value || "";
      } else {
        draft[k] = (entry.el.value || "").trim();
      }
    });
    /* Keyword auto-fills from name when blank, mirroring charm save. */
    if (!draft.lorebookKeyword) draft.lorebookKeyword = draft.name;
...
function mrrpP3RenderBackgroundsSection(parent) {
  ...
  mrrpP3RenderSection(parent, {
    ...
    entries.forEach(function (entry, idx) {
      /* Phase 7 — row layout: name (read-only span, click to edit) +
         dots (number input, kept editable inline for quick adjustment)
         + ✎ edit (opens full dialog) + × remove. Hover the name span
         to see the description as a tooltip. Old inline-name-input
         behaviour replaced — the dialog owns name/description/mechanical
         editing now. */
      var row = marinara.addElement(body, "div", { "class": "mrrp-skill-spec-row mrrp-trait-row" });
      if (!row) return;
      var tooltip = "";
      if (entry.description) tooltip = entry.description;
      if (entry.mechanical)  tooltip = (tooltip ? tooltip + "\n\n— Mechanical —\n" : "") + entry.mechanical;
      var nameSpan = marinara.addElement(row, "span", {
        "class": "mrrp-skill-spec-name mrrp-trait-row__name",
        textContent: entry.name || (textOnly ? "(unnamed feat)" : "(unnamed background)"),
        title: tooltip || (textOnly ? "Click ✎ to edit this feat" : "Click ✎ to edit this background")
      });
      if (nameSpan) marinara.on(nameSpan, "click", function (e) {
        if (e && typeof e.stopPropagation === "function") e.stopPropagation();
        openBackgroundDialog(idx);
      });
      if (!textOnly) {
        var valInput = marinara.addElement(row, "input", {
          "class": "mrrp-p3-row__val",
        ...
        if (valInput) {
          ...
          marinara.on(valInput, "change", function () {
            ...
            entry.value = n;
            saveSheet(state.chatId, state.sheet);
            /* Re-upsert so the lorebook content reflects the new dot
               count (the entry head includes "(Background — N dots)"). */
            upsertSheetEntryLorebook("background", entry, state.activeCharacterId)
              .catch(function (e) { log("background dots-change upsert failed", e && e.message); });
            renderSheet();
          });
        }
      }
      var editBtn = marinara.addElement(row, "button", {
        type: "button",
        "class": "mrrp-char-btn mrrp-char-btn--icon",
        textContent: "✎",
        title: "Edit " + (textOnly ? "feat" : "background")
      });
      if (editBtn) marinara.on(editBtn, "click", function (e) {
        if (e && typeof e.stopPropagation === "function") e.stopPropagation();
        openBackgroundDialog(idx);
      });
      var removeBtn = marinara.addElement(row, "button", {
        type: "button",
        "class": "mrrp-char-btn mrrp-char-btn--icon mrrp-char-btn--danger",
        textContent: "×",
...
function addMeritFlaw(kind) {
  if (!state.sheet) return;
  /* Phase 7 — Add now opens the dialog; entry materialises on Save. */
  openMeritFlawDialog(null, (kind === "flaw") ? "flaw" : "merit");
}
...
function removeMeritFlaw(id) {
  ...
  for (var i = 0; i < state.sheet.meritsFlaws.length; i++) {
    if (state.sheet.meritsFlaws[i] && state.sheet.meritsFlaws[i].id === id) {
      var removed = state.sheet.meritsFlaws[i];
      state.sheet.meritsFlaws.splice(i, 1);
      saveSheet(state.chatId, state.sheet);
      if (removed && removed.lorebookEntryId) {
        deleteSheetEntryLorebook(removed).catch(function (e) {
          log("merit/flaw remove: lorebook delete failed", e && e.message);
        });
      }
      renderSheet();
      return;
...
/* Phase 7 — Merit/Flaw edit dialog. Same shape as the background dialog
   plus a kind+type selector (Merit/Flaw is a tab; type is Physical/
   Mental/Social/Supernatural). Save upserts to the chat lorebook. */
function openMeritFlawDialog(id, defaultKind) {
  if (!Array.isArray(state.sheet.meritsFlaws)) state.sheet.meritsFlaws = [];
  var existing = null;
  if (id) existing = state.sheet.meritsFlaws.find(function (e) { return e && e.id === id; }) || null;
  var draft = existing
    ? JSON.parse(JSON.stringify(existing))
    : {
        id: newMeritFlawId(),
        kind: (defaultKind === "flaw") ? "flaw" : "merit",
        name: "",
        type: "physical",
        points: 1,
        description: "",
  ...
  var kindLbl = (draft.kind === "flaw") ? "Flaw" : "Merit";
  openTraitDialog({
    title: existing ? ("Edit " + kindLbl) : ("New " + kindLbl),
    kind: draft.kind,
    draft: draft,
    fields: [
      { key: "name",        label: "Name",  type: "text",     placeholder: kindLbl === "Merit" ? "merit (e.g. Eat Food, Iron Will)" : "flaw (e.g. Lunatic, Repulsed by Garlic)" },
      { key: "kind",        label: "Kind",  type: "select",   options: [{ value: "merit", label: "Merit" }, { value: "flaw", label: "Flaw" }] },
      { key: "type",        label: "Type",  type: "select",   options: MERIT_FLAW_TYPES.map(function (t) { return { value: t.id, label: t.label }; }) },
      { key: "points",      label: "Points",type: "number",   min: 1, max: 7, "default": 1 },
      { key: "description", label: "Description",        type: "textarea", placeholder: "What this " + kindLbl.toLowerCase() + " represents in the fiction." },
      { key: "mechanical",  label: "Mechanical",         type: "textarea", placeholder: "Mechanical effect — bonus dice, conditions, story hooks…" },
      { key: "lorebookKeyword", label: "Keyword", type: "text",   placeholder: "(auto-fills from name on save)" }
    ],
    onSave: function (saved) {
      if (!Array.isArray(state.sheet.meritsFlaws)) state.sheet.meritsFlaws = [];
      var idx = state.sheet.meritsFlaws.findIndex(function (e) { return e && e.id === saved.id; });
      if (idx >= 0) state.sheet.meritsFlaws[idx] = saved;
      else state.sheet.meritsFlaws.push(saved);
      saveSheet(state.chatId, state.sheet);
      renderSheet();
      upsertSheetEntryLorebook(saved.kind === "flaw" ? "flaw" : "merit", saved, state.activeCharacterId).then(function (entryId) {
        if (entryId && entryId !== saved.lorebookEntryId) {
          saved.lorebookEntryId = entryId;
          var arr = state.sheet.meritsFlaws;
          var hit = arr && arr.find(function (e) { return e && e.id === saved.id; });
          if (hit) { hit.lorebookEntryId = entryId; saveSheet(state.chatId, state.sheet); }
        }
      }).catch(function (e) { log("merit/flaw upsert failed", e && e.message); });
    }
  });
}
  ...
  mrrpP3RenderSection(parent, {
    ...
    ["merit", "flaw"].forEach(function (kind) {
      ...
      entries.forEach(function (entry) {
        ...
        /* Phase 7 — row shows the trait read-only, click ✎ for the full
           dialog. Name is a clickable span (hover for description), type
           shows as a small static label, points stays as an editable
           inline number for quick adjustment. Saves to lorebook on
           point change as well as on dialog save. */
        var tooltip = "";
        if (entry.description) tooltip = entry.description;
        if (entry.mechanical)  tooltip = (tooltip ? tooltip + "\n\n— Mechanical —\n" : "") + entry.mechanical;
        var nameSpan = marinara.addElement(row, "span", {
          "class": "mrrp-skill-spec-name mrrp-trait-row__name",
          textContent: entry.name || (kind === "merit" ? "(unnamed merit)" : "(unnamed flaw)"),
          title: tooltip || ("Click ✎ to edit this " + (kind === "merit" ? "merit" : "flaw"))
        });
        if (nameSpan) marinara.on(nameSpan, "click", function (e) {
          if (e && typeof e.stopPropagation === "function") e.stopPropagation();
          openMeritFlawDialog(entry.id);
        });

        var typeLbl = (entry.type || "physical");
        typeLbl = typeLbl.charAt(0).toUpperCase() + typeLbl.slice(1);
        marinara.addElement(row, "span", {
          "class": "mrrp-trait-row__type",
          textContent: typeLbl,
          title: "Type — edit via ✎"
        });
        ...
        if (ptsInput) {
          ...
          marinara.on(ptsInput, "change", function () {
            ...
            saveSheet(state.chatId, state.sheet);
            upsertSheetEntryLorebook(entry.kind === "flaw" ? "flaw" : "merit", entry, state.activeCharacterId)
              .catch(function (e) { log("merit/flaw points-change upsert failed", e && e.message); });
          });
        }
        ...
        var editBtn = marinara.addElement(row, "button", {
          ...
          "class": "mrrp-char-btn mrrp-char-btn--icon",
          textContent: "✎",
          title: "Edit " + (kind === "merit" ? "merit" : "flaw")
        });
        if (editBtn) marinara.on(editBtn, "click", function (e) {
          if (e && typeof e.stopPropagation === "function") e.stopPropagation();
          openMeritFlawDialog(entry.id);
...
function deleteItem(id) {
  ...
  saveSheet(state.chatId, state.sheet);
  /* Phase 7 — clean up the removed item's chat-lorebook entry so its
     description / mechanical text stops injecting into AI context. */
  if (removed && removed.lorebookEntryId) {
    deleteSheetEntryLorebook(removed).catch(function (e) {
      log("item delete: lorebook delete failed", e && e.message);
    });
  }
}
...
function openItemDialog(itemId, onSaved, defaultCategory) {
  ...
  /* Phase 7 — Description + Mechanical + Keyword. The Description /
     Mechanical pair fuels the chat Character Reference lorebook entry
     so the AI sees this gear when the keyword fires in narration. The
     existing Notes textarea above is preserved as player-side scratch
     space that is NOT injected into the AI's context — useful for
     reminders the AI shouldn't see ("the dagger is poisoned but the
     party doesn't know"). */
  ...
  var descInput = marinara.addElement(descRow, "textarea", {
    "class": "mrrp-item-form__textarea",
    placeholder: "What the item looks like and what it means in the fiction (heirloom blade, blessed by your sept's totem…)."
  });
  if (descInput) descInput.value = draft.description || "";

  var mechRow = marinara.addElement(dialog, "div", { "class": "mrrp-item-form__row" });
  marinara.addElement(mechRow, "label", { textContent: "Mechanical" });
  var mechInput = marinara.addElement(mechRow, "textarea", {
    "class": "mrrp-item-form__textarea",
    placeholder: "Mechanical effect — special properties, charges, conditions of use…"
  });
  if (mechInput) mechInput.value = draft.mechanical || "";
  ...
  if (buttons) {
    ...
    if (btnSave) marinara.on(btnSave, "click", function () {
      ...
      /* Phase 7 — read the new Description / Mechanical / Keyword inputs
         into the draft, mirroring the trait-dialog convention. Keyword
         auto-fills from name when blank so the lorebook entry has a
         trigger string. */
      draft.description = (descInput && descInput.value || "").trim();
      draft.mechanical  = (mechInput && mechInput.value || "").trim();
      ...
      /* Phase 7 — upsert the item into the chat Character Reference
         lorebook so the AI can read its description, mechanical text,
         and bonuses when the keyword fires in narration. Persist the
         returned lorebookEntryId so future saves PATCH the same entry. */
      upsertSheetEntryLorebook("item", draft, state.activeCharacterId).then(function (entryId) {
        if (entryId && entryId !== draft.lorebookEntryId) {
          draft.lorebookEntryId = entryId;
          var arr = state.sheet.inventory;
          var hit = arr && arr.find(function (it) { return it.id === draft.id; });
          if (hit) { hit.lorebookEntryId = entryId; saveSheet(state.chatId, state.sheet); }
        }
      }).catch(function (e) { log("item upsert failed", e && e.message); });
...
function renderSpellbookContents() {
  ...
  if (!state.sheet.hiddenAbilityCategories || typeof state.sheet.hiddenAbilityCategories !== "object") {
    state.sheet.hiddenAbilityCategories = {};
  }

  /* Phase 7 — Re-sync to chat lorebook button. Self-heal for two real
     scenarios:
     (1) Player adds charms while on the application home screen (no
         active chat → state.chatId null → upsertAbilityLorebookEntry
         silently no-ops because there's no chat to scope the spellbook
         lorebook to).
     (2) Same character imported into a new chat — abilities live on the
         sheet but the new chat's Player Spellbook is empty.
     Click the button: iterate every ability on the active character and
     re-run the upsert. Idempotent — existing entries PATCH; missing
     entries fall back to POST via the existing 404 handler. */
  var syncRow = marinara.addElement(body, "div", { "class": "mrrp-spellbook__sync-row" });
  if (syncRow) {
    var syncBtn = marinara.addElement(syncRow, "button", {
      "class": "mrrp-char-btn mrrp-char-btn--dashed mrrp-spellbook__sync-btn",
      type: "button",
      textContent: "Re-sync to chat lorebook",
      title: "Re-create this chat's Player Spellbook entries from your sheet. Useful if you added charms before opening a chat, or if you switched chats with the same character."
    });
    var syncMsg = marinara.addElement(syncRow, "span", {
      "class": "mrrp-spellbook__sync-msg",
      textContent: ""
    });
    function setSyncMsg(text, kind) {
      if (!syncMsg) return;
      syncMsg.textContent = text || "";
      syncMsg.classList.remove("mrrp-spellbook__sync-msg--ok", "mrrp-spellbook__sync-msg--warn", "mrrp-spellbook__sync-msg--err");
      if (kind) syncMsg.classList.add("mrrp-spellbook__sync-msg--" + kind);
    }
    if (syncBtn) marinara.on(syncBtn, "click", function () {
      if (!state.chatId) {
        setSyncMsg("Open a chat first — the spellbook is per-chat.", "warn");
        return;
      }
      var charId = state.activeCharacterId;
      var jobs = [];
      /* Phase 7 — sync every sheet-entry kind that backs onto the
         Character Reference lorebook: abilities (charms / gifts /
         disciplines), backgrounds, merits/flaws, and inventory items.
         Each job returns true if a new entry was created, false if an
         existing entry was refreshed, null on failure. */
      function pushJob(kind, entry, extraCtx) {
        if (!entry) return;
        jobs.push(upsertSheetEntryLorebook(kind, entry, charId, extraCtx).then(function (entryId) {
          if (entryId && entryId !== entry.lorebookEntryId) {
            entry.lorebookEntryId = entryId;
            return true;
          }
          return false;
        }).catch(function (e) {
          log("re-sync: upsert failed for " + (entry.name || entry.id || "(unnamed)"), e && e.message);
          return null;
        }));
      }
      var abilitiesByCat = state.sheet.abilities || {};
      Object.keys(abilitiesByCat).forEach(function (catId) {
        var arr = abilitiesByCat[catId];
        if (!Array.isArray(arr)) return;
        arr.forEach(function (ab) {
          if (ab && (ab.id || ab.name)) pushJob("ability", ab, { catId: catId });
        });
      });
      (state.sheet.backgrounds || []).forEach(function (bg) {
        if (bg && bg.name) pushJob("background", bg);
      });
      (state.sheet.meritsFlaws || []).forEach(function (mf) {
        if (mf && mf.name) pushJob(mf.kind === "flaw" ? "flaw" : "merit", mf);
      });
      (state.sheet.inventory || []).forEach(function (item) {
        if (item && item.name) pushJob("item", item);
      });
      if (!jobs.length) {
        setSyncMsg("Nothing on the sheet to sync — add a charm, background, merit/flaw, or item first.", "warn");
        return;
      }
      setSyncMsg("Syncing " + jobs.length + " entries…", null);
      syncBtn.disabled = true;
      Promise.all(jobs).then(function (results) {
        syncBtn.disabled = false;
        var newCount = results.filter(function (r) { return r === true; }).length;
  ...
  /* Phase 7 — per-ruleset and per-sheet category hiding.
     - cfg.hideEmptyCategories: ruleset opts in to auto-hiding categories
       that currently hold zero abilities (re-shown when the player adds
       one). WoD-pattern rulesets (vtmv20, exwod, w20, exalted3e) set this.
     - state.sheet.hiddenAbilityCategories[id]: per-sheet user toggle —
       click the small × on a category header to hide it; an "(N hidden)
       show all" link at the flyout bottom restores them.
     Categories with abilities are NEVER auto-hidden; only the "currently
     empty" and "user-hidden" cases drop out. */
  var hideEmpty = !!(cfg && cfg.hideEmptyCategories);
  var hiddenCount = 0;

  var declaredIds = {};
  cfg.categories.forEach(function (cat) {
    declaredIds[cat.id] = true;
    var abs = state.sheet.abilities[cat.id];
    var hasAbilities = Array.isArray(abs) && abs.length > 0;
    if (state.sheet.hiddenAbilityCategories[cat.id]) { hiddenCount += 1; return; }
    if (hideEmpty && !hasAbilities) { return; }
  ...
  /* Phase 7 — when the player has hidden one or more categories via the
     × on the category head, show a small link to bring them back. */
  if (hiddenCount > 0) {
    var showHiddenBtn = marinara.addElement(body, "button", {
      "class": "mrrp-char-btn mrrp-char-btn--dashed mrrp-spellbook__show-hidden",
      type: "button",
      textContent: "Show " + hiddenCount + " hidden " + (hiddenCount === 1 ? "category" : "categories")
    });
    if (showHiddenBtn) marinara.on(showHiddenBtn, "click", function () {
      state.sheet.hiddenAbilityCategories = {};
      saveSheet(state.chatId, state.sheet);
      renderSpellbookContents();
    });
...
function renderSpellbookCategory(body, cat, orphanCategoryIds, isCustom) {
  ...
  /* Phase 7 — the per-category 0-10 rating input is only meaningful for
     rulesets whose abilities ARE rated per-line (vtmv20 Disciplines:
     Animalism 3 etc.). For rulesets where Charms/Gifts are individual
     powers (exalted3e, exwod, w20), the input is noise — opt out via
     abilities.showCategoryScores: false. Default true preserves vtmv20. */
  var abilitiesCfgForScore = state.ruleset && state.ruleset.abilities;
  var showCategoryScores = !abilitiesCfgForScore || abilitiesCfgForScore.showCategoryScores !== false;
  if (isPoolMode && !orphanCategoryIds && showCategoryScores) {
  ...
  /* Phase 7 — per-sheet hide button for declared (non-custom, non-orphan)
     categories. Click to add the category to state.sheet.hiddenAbility-
     Categories; the parent flyout shows a "Show N hidden" link to restore. */
  if (!isCustom && !orphanCategoryIds) {
    var hideBtn = marinara.addElement(headRow, "button", {
      "class": "mrrp-char-btn mrrp-char-btn--icon mrrp-spellbook-cat__hide",
      type: "button",
      textContent: "×",
      title: "Hide this category from the flyout (click the 'Show N hidden' link to restore)"
    });
    if (hideBtn) marinara.on(hideBtn, "click", function (e) {
      if (e && typeof e.stopPropagation === "function") e.stopPropagation();
      if (!state.sheet.hiddenAbilityCategories || typeof state.sheet.hiddenAbilityCategories !== "object") {
        state.sheet.hiddenAbilityCategories = {};
      }
      state.sheet.hiddenAbilityCategories[cat.id] = true;
      saveSheet(state.chatId, state.sheet);
      renderSpellbookContents();
    });
  }
...
function renderAbilityRow(list, ab, catId) {
  ...
  /* Action buttons — Phase 7 reshuffle: Cast is the primary action and
     gets a wide accented button; Edit and Delete are icon-sized so the
     row stays on one line. Cast still renders only when meaningful for
     the ruleset's resolution mode. */
  ...
  if (state.ruleset && state.ruleset.resolution &&
    ...
    var castBtn = marinara.addElement(row, "button", {
      "class": "mrrp-char-btn mrrp-char-btn--accent mrrp-char-btn--cast",
      type: "button",
      textContent: "Cast",
      title: isPoolModeRow
        ? "Announce activation in chat with name + cost; pre-fills the dice widget when Cast attr/skill/difficulty are set"
  ...
  var delBtn = marinara.addElement(row, "button", {
    "class": "mrrp-char-btn mrrp-char-btn--danger mrrp-char-btn--icon",
    type: "button",
    textContent: "×",
    title: "Delete"
  });
...
function castAbilityPool(ability, catId) {
  ...
  /* Open the dice widget so the player can immediately roll their
     activation pool. Phase 7: when the ability declares castAttribute +
     castSkill, compute pool = attr value + skill value and pre-fill.
     Falls back to the legacy category-rating pre-fill for vtmv20
     Disciplines etc. Difficulty pre-fills the target face. The special
     dropdown values "_willpower_current" and "_willpower_permanent"
     resolve to the Willpower pool current / Permanent Willpower stat. */
  if (state.ruleset.resolution && state.ruleset.resolution.mode === MODES.POOL) {
    showDice(true);
    if (typeof setDiceInput === "function") {
      var ctx = (typeof statContext === "function") ? statContext() : {};
      function resolveAttrValue(name) {
        if (!name) return 0;
        if (name === "_willpower_current") {
          /* current pool: state.sheet.derived["Willpower"] is the
             temporary current set by Permanent Willpower binding. */
          var wp = (state.sheet && state.sheet.derived && typeof state.sheet.derived["Willpower"] === "number")
            ? state.sheet.derived["Willpower"] : 0;
          return wp;
        }
        if (name === "_willpower_permanent") {
          var perm = (state.sheet && state.sheet.derived && typeof state.sheet.derived["Permanent Willpower"] === "number")
            ? state.sheet.derived["Permanent Willpower"] : 0;
          return perm;
        }
        if (typeof ctx[name] === "number") return ctx[name];
        if (state.sheet && state.sheet.attributes && typeof state.sheet.attributes[name] === "number") {
          return state.sheet.attributes[name];
        }
        return 0;
      }
      function resolveSkillValue(name) {
        if (!name) return 0;
        if (typeof ctx[name] === "number") return ctx[name];
        if (state.sheet && state.sheet.skills && typeof state.sheet.skills[name] === "number") {
          return state.sheet.skills[name];
        }
        return 0;
      }
      var poolFilled = false;
      if (ability.spellcastingAttribute) {
        var attrVal = resolveAttrValue(ability.spellcastingAttribute);
        var skillVal = resolveSkillValue(ability.castSkill);
        var poolFromFields = attrVal + skillVal;
        if (poolFromFields > 0) {
          setDiceInput("pool", poolFromFields);
          poolFilled = true;
        }
      }
      if (!poolFilled && rating) {
        var poolFromRating = parseInt(rating, 10);
        if (isFinite(poolFromRating) && poolFromRating > 0) setDiceInput("pool", poolFromRating);
      }
      if (ability.difficulty) {
        var d = parseInt(String(ability.difficulty), 10);
        if (isFinite(d) && d > 0) setDiceInput("target", d);
      }
    }
  }
...
function openAbilityDialog(abilityId, defaultCategoryId, onSaved) {
  ...
  var draft = existing
    ...
    : {
        ...
        /* Cast-time fields. damageDice parses via the same NdM[+K type]
           ...
           D&D 5e "save halves" damage rule. Phase 7: castSkill / saveSkill
           pair with their attribute counterparts for WoD-pattern
           Attribute+Skill pools, and difficulty pre-fills the dice
           widget's target face on Cast. */
        damageDice: "",
        saveAttribute: "",
  ...
  if (typeSel) {
    /* Phase 7 — expanded type list. Original D&D-flavoured options
       (passive / triggered / at-will / per-rest) retained; new WoD/Exalted-
       flavoured options (reflexive / action / ritual) appended. If a
       ruleset author wants a custom list, declare abilities.types[] on
       the ruleset and that wins. */
    var abilitiesCfgForTypes = state.ruleset.abilities || {};
    var typeOptions = Array.isArray(abilitiesCfgForTypes.types) && abilitiesCfgForTypes.types.length
      ? abilitiesCfgForTypes.types
      : ["passive", "triggered", "at-will", "per-rest", "reflexive", "action", "ritual"];
    /* If the saved draft.type isn't in the options (e.g. legacy "at-will"
       on a ruleset that overrides types), keep it visible at the top so
       editing doesn't silently change the value. */
    if (draft.type && typeOptions.indexOf(draft.type) === -1) {
      typeOptions = [draft.type].concat(typeOptions);
    }
  ...
  /* Phase 7 — spell-style fields (Damage, Save target, Half-on-save) are
     D&D-shaped and noise for WoD-pattern rulesets where the same data
     lives in the description. A ruleset opts out via
     abilities.hideSpellFields: true (exwod, w20, exalted3e set it).
     The legacy Damage / Cast attr (single) / Save vs (single) / Half-on-
     save block is preserved when the flag is unset so D&D bundles keep
     working unchanged. */
  var abilitiesCfg = state.ruleset.abilities || {};
  var hideSpellFields = !!abilitiesCfg.hideSpellFields;
  ...
  if (!hideSpellFields) {
    ...
    effectInput = marinara.addElement(effectRow, "input", {
      "class": "mrrp-item-form__input",
      type: "text",
    ...
    dmgInput = marinara.addElement(dmgRow, "input", {
      "class": "mrrp-item-form__input",
      type: "text",
  ...
  /* Helper: build an attribute dropdown that includes the special
     "Willpower (current)" and "Permanent Willpower" choices at the top
     for WoD pools. Stored values: "_willpower_current" / "_willpower_-
     permanent" — castAbilityPool reads these and resolves to the
     Willpower pool current or the Permanent Willpower derived stat
     respectively. */
  function buildAttrDropdown(rowEl, currentValue) {
    var sel = marinara.addElement(rowEl, "select", { "class": "mrrp-item-form__select" });
    if (!sel) return null;
    var blank = document.createElement("option");
    blank.value = ""; blank.textContent = "—";
    sel.appendChild(blank);
    var wpCur = document.createElement("option");
    wpCur.value = "_willpower_current"; wpCur.textContent = "Willpower (current)";
    if (currentValue === "_willpower_current") wpCur.selected = true;
    sel.appendChild(wpCur);
    var wpPerm = document.createElement("option");
    wpPerm.value = "_willpower_permanent"; wpPerm.textContent = "Permanent Willpower";
    if (currentValue === "_willpower_permanent") wpPerm.selected = true;
    sel.appendChild(wpPerm);
    (state.ruleset.attributes || []).forEach(function (a) {
      var opt = document.createElement("option");
      opt.value = a.name; opt.textContent = a.name;
      if (currentValue === a.name) opt.selected = true;
      sel.appendChild(opt);
    });
    return sel;
  }

  function buildSkillDropdown(rowEl, currentValue) {
    var sel = marinara.addElement(rowEl, "select", { "class": "mrrp-item-form__select" });
    if (!sel) return null;
    var blank = document.createElement("option");
    blank.value = ""; blank.textContent = "—";
    sel.appendChild(blank);
    (state.ruleset.skills || []).forEach(function (s) {
      var opt = document.createElement("option");
  ...
  /* Phase 7 — Cast pool = Attr + Skill. The Skill side is optional;
     leaving it blank gives Attr-only (the legacy D&D shape). Both sides
     accept the special Willpower choices via the attr dropdown only
     (skills don't get them — Willpower is conceptually attribute-shaped). */
  ...
  var castAttrSel = buildAttrDropdown(castAttrRow, draft.spellcastingAttribute);

  var castSkillRow = marinara.addElement(dialog, "div", { "class": "mrrp-item-form__row" });
  marinara.addElement(castSkillRow, "label", { textContent: "Cast skill" });
  var castSkillSel = buildSkillDropdown(castSkillRow, draft.castSkill);

  var saveAttrRow = marinara.addElement(dialog, "div", { "class": "mrrp-item-form__row" });
  marinara.addElement(saveAttrRow, "label", { textContent: "Save vs (attr)" });
  var saveAttrSel = buildAttrDropdown(saveAttrRow, draft.saveAttribute);

  var saveSkillRow = marinara.addElement(dialog, "div", { "class": "mrrp-item-form__row" });
  marinara.addElement(saveSkillRow, "label", { textContent: "Save vs (skill)" });
  var saveSkillSel = buildSkillDropdown(saveSkillRow, draft.saveSkill);

  var difficultyRow = marinara.addElement(dialog, "div", { "class": "mrrp-item-form__row" });
  marinara.addElement(difficultyRow, "label", { textContent: "Difficulty" });
  var difficultyInput = marinara.addElement(difficultyRow, "input", {
    "class": "mrrp-item-form__input",
    type: "text",
    value: draft.difficulty || "",
    placeholder: "6 (pool target face) · 15 (DC) — pre-fills the dice widget on Cast"
  });
  ...
  if (!hideSpellFields) {
    ...
    halfInput = marinara.addElement(halfRow, "input", { type: "checkbox" });
  ...
  if (saveBtn) marinara.on(saveBtn, "click", function () {
    ...
    /* effectText / damageDice / halfOnSave only writeable when the
       form rendered the fields (hideSpellFields off). Otherwise leave
       any previously-stored values alone — round-tripping a hidden
       field through Save shouldn't wipe it. */
    if (effectInput) draft.effectText = (effectInput.value || "").trim();
    if (dmgInput)    draft.damageDice = (dmgInput.value || "").trim();
    if (halfInput)   draft.halfOnSave = !!halfInput.checked;
    draft.description = (descInput && descInput.value || "").trim();
    draft.lorebookKeyword = kw;
    draft.spellcastingAttribute = (castAttrSel && castAttrSel.value) || "";
    draft.saveAttribute = (saveAttrSel && saveAttrSel.value) || "";
    /* Phase 7 — Cast+Save skill pair and difficulty for WoD-pattern
       (Attribute + Skill) pool resolution and dice-widget pre-fill. */
    draft.castSkill = (castSkillSel && castSkillSel.value) || "";
    draft.saveSkill = (saveSkillSel && saveSkillSel.value) || "";
    draft.difficulty = (difficultyInput && difficultyInput.value || "").trim();
  ...
  return apiFetch("/lorebooks").then(function (lbs) {
    ...
    var body = {
      /* Phase 7 — the lorebook now houses charms AND backgrounds, merits,
         ...
         Reference" so the display matches its broader scope. Tag stays
         MRR_TAG_SPELLBOOK for backward compat with existing lorebooks. */
      name: getActiveChatTitle() + " — Character Reference",
      description: "Player-authored character traits for this chat — charms, backgrounds, merits/flaws, and items. Auto-managed by Marinara-RPG-RP-Mode-Extension; safe to edit by hand.",
      category: "spellbook",
      chatId: state.chatId,
...
function abilityEntryBody(ability, charId, catId) {
  ...
  function _abilityAttrLabel(v) {
    ...
    if (v === "_willpower_permanent") return "Permanent Willpower";
    return v;
  }
  var castParts = [];
  if (ability.damageDice) castParts.push("Damage: " + ability.damageDice);
  if (ability.spellcastingAttribute) castParts.push("Cast attribute: " + _abilityAttrLabel(ability.spellcastingAttribute));
  if (ability.castSkill) castParts.push("Cast skill: " + ability.castSkill);
  if (ability.saveAttribute) castParts.push("Save vs (attr): " + _abilityAttrLabel(ability.saveAttribute));
...
/* Phase 7 — body builders for non-ability sheet entries that share the
   chat-scoped Character Reference lorebook. Mirror the abilityEntryBody
   shape: tags include MRR_TAG_SPELLBOOK so existing scan code finds them,
   plus a kind-discriminator tag ("mrr-background", "mrr-meritflaw",
   "mrr-item") for future per-kind operations (filtering, bulk cleanup). */
...
function bgEntryBody(bg, charId) {
  ...
  return {
    ...
    enabled: true,
    role: "system",
    tags: [MRR_TAG_SPELLBOOK, MRR_TAG_CHAR_PFX + (charId || "unknown"), "mrr-background"]
  };
}

function mfEntryBody(mf, charId) {
  var keyword = (mf.lorebookKeyword || mf.name || "").trim();
  var kindLbl = (mf.kind === "flaw") ? "Flaw" : "Merit";
  ...
  return {
    ...
    enabled: true,
    role: "system",
    tags: [MRR_TAG_SPELLBOOK, MRR_TAG_CHAR_PFX + (charId || "unknown"), "mrr-meritflaw"]
  };
}

function itemEntryBody(item, charId) {
  ...
  if (Array.isArray(item.bonuses) && item.bonuses.length) {
    var bonusStrs = item.bonuses
      .filter(function (b) { return b && b.target; })
      .map(function (b) {
        var v = (typeof b.value === "number") ? b.value : 0;
        return "  " + b.target + " " + (v >= 0 ? "+" + v : v) + " " + (b.kind || "value") + (b.tag ? " (" + b.tag + ")" : "");
      });
    if (bonusStrs.length) content += "\n\nEquipped bonuses:\n" + bonusStrs.join("\n");
  }
  ...
  return {
    ...
    enabled: true,
    role: "system",
    tags: [MRR_TAG_SPELLBOOK, MRR_TAG_CHAR_PFX + (charId || "unknown"), "mrr-item"]
  };
}
...
/* Phase 7 — thin compat wrappers around the new generic helpers. The
   original implementations have been folded into upsertSheetEntryLorebook
   and deleteSheetEntryLorebook above; keeping these names lets existing
   call sites stay unchanged and signals that the charm path is the
   reference shape every other sheet-entry kind follows. */
function upsertAbilityLorebookEntry(ability, charId, catId) {
  return upsertSheetEntryLorebook("ability", ability, charId, { catId: catId });
}

function deleteAbilityLorebookEntry(ability) {
  return deleteSheetEntryLorebook(ability);
}
